...
class Installation extends Resource
{
    ...
    {
        return __('Instalaciones Superficiales');
    }


    public static function singularLabel()
    {
        return __('Instalación Superficial');
    }
     ...
     */
    public static $model = \App\Models\Installation::class;
    ...
    public function inmuebleFields()
    {
        return [
            ...
            Text::make('Ejido', 'ejido_inmueble')->showOnUpdating(fn () => Auth::user()->role === 'admin' || Auth::user()->role === 'gestor')->hideFromIndex()->readonly(fn (NovaRequest $r) => $this->validateEditionField($r))->size('w-1/3')->hide()->dependsOn(
                ...
                function (Text $field, NovaRequest $request, FormData $formData) {
                    $rpi = $formData->regimen_propiedad_inmueble;
                    if ($rpi === 'ej' || $rpi === 'pa' || $rpi === 'po') {
                        $field->show();
                    }
                }
            )->hideFromDetail(function (NovaRequest $request) {
                $rpi = $this->getRawOriginal('regimen_propiedad_inmueble');
                if ($rpi === 'pr' || $rpi === 'pa' || $rpi === 'po') {
                    return true;
                }
                return false;
            }),
